...
var disp_columns = ['Claim Number','Client ID','Vendor ID','State','Claim Type','Date of Loss (YYYYMMDD)','Body Part','Legal Indicator','Indemnity Cost (USD)','Medical Cost (USD)','Expense Cost (USD)'];
function disp_table()
{			document.getElementById("searchComplete").innerHTML = "Data is loading: We assure you, it's worth the wait!" ;
			...
			$(window).resize(function() {
				...
				//CEHS_Dashboard_data_table_test
...
function tabulate(data,columns, input_LegalInd, input_state, input_year, input_BodyPart) {
				...
				var tablebody = d3.select('#tablecontainer')
					...
					document.getElementById("searchComplete").innerHTML = "Data is loaded" ;
...
function filter_table()
{
	...
	else if(input_LegalInd == "N"){
		var_case = "NonLitigated";
	}
	else{
		var_case = "All";
	}
	
	tabulate(copy_source_data,columns, var_case, input_state, input_year, input_BodyPart );
